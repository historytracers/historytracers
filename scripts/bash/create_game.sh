...
ht_create_game_files () {
    CT=$(date +%s)
    cd ../../lang || exit
    find ./* \( -name "??-??" \) -exec bash -c 'if [ ! -d "$1/smGame/" ]; then mkdir "$1/smGame/"; fi; cp ../src/json/scientific_method_game_template.json "$1/smGame/$2.json"; sed -i "s/File containing all the sources referenced in the text./$2/g" "$1/smGame/$2.json"; sed -i "s/The time of the last file update, represented as Unix Epoch time./$3/g" "$1/smGame/$2.json"' shell {} "$1" "$CT" \;
}
