...
function htSorobanReset() { 
    if (localSorobanController.abacusMode === 'schyoty') {
        htSchyotyReset();
        return;
    }
    for(let i=0;i<localSorobanController.COLUMNS;i++){ localSorobanController.state[i].upper=0; localSorobanController.state[i].lower=0; } htSorobanRender(); htSorobanUpdateDisplay();}
...
function htSchyotySetToNumber(value) {
    ...
    for (let r = 0; r < HT_SCHYOTY_ROWS; r++) {
        const digitIdx = str.length - 1 - r;
        localSorobanController.schyotyState[r] = digitIdx >= 0 ? parseInt(str[digitIdx]) : 0;
    }
    htSchyotyRender();
    htSorobanUpdateDisplay();
}
...
function htSchyotyHandleCanvasStart(e) {
    ...
    if (hit) {
        if (!localSorobanController.finalCongratsShown) setControlButtonsVisibility(false);
...
function buildStepsForNumbers(a, b) {
    ...
    stepsList.push({
        instruction: localSorobanController.TextManager.getStep1Instruction(localSorobanController.TextManager.formatNumber(a), placeDescription),
        targetValue: a
    });
    ...
    for (let p = 0; p < maxDigits; p++) {
        ...
        if (total < 10) {
            ...
            stepsList.push({
                instruction: localSorobanController.TextManager.getSimpleAddInstruction(placeNames[p], digitB, localSorobanController.TextManager.formatNumber(currentValue)),
            ...
            stepsList.push({
                instruction: localSorobanController.TextManager.getCarryInstruction(placeNames[p], digitB, digitA, total, nextPlace, complement, localSorobanController.TextManager.formatNumber(newValue)),
    ...
    stepsList.push({
        instruction: localSorobanController.TextManager.getFinalInstruction(localSorobanController.TextManager.formatNumber(a), localSorobanController.TextManager.formatNumber(b), localSorobanController.TextManager.formatNumber(a+b)),
        targetValue: a + b
    });
