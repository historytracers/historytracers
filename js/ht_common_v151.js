...
switch(page){case"tree":case"genealogical_map_list":case"class_content":if(reload==true&&lastTreeLoaded.arg!=null&&lastTreeLoaded.arg.length>0){arg=lastTreeLoaded.arg;htHistoryIdx.clear();htLiteratureIdx.clear();htFirstStepsIdx.clear();}else{lastTreeLoaded.page=page;lastTreeLoaded.arg=arg;}
...
{if(index=="history"){return htHistoryIdx;}else if(index=="literature"){return htLiteratureIdx;}else if(index=="first_steps"){return htFirstStepsIdx;}
return undefined;}
...
var prev="";if(ptr.prev==index){prev="<a href=\"index.html?page="+index+"\" onclick=\"htLoadPage('"+index+"','html', '', false); return false;\"><span>"+keywords[60]+"</span></a>";}else{var prevPtr=idx.get(ptr.prev);prev="<a href=\"index.html?page=class_content&arg="+ptr.prev+"\" onclick=\"htLoadPage('class_content', 'html', '"+ptr.prev+"', false); return false;\">"+prevPtr.name+"</a>";}
var next="";if(ptr.next==undefined){next="&nbsp;";}else{var nextPtr=idx.get(ptr.next);next="<a href=\"index.html?page=class_content&arg="+ptr.next+"\" onclick=\"htLoadPage('class_content', 'html', '"+ptr.next+"', false); return false;\">"+nextPtr.name+"</a>";}
...
var table=data.content[i].value;for(const j in table){var fillNext=idx.get(prev);if(fillNext!=undefined){fillNext.next=table[j].id;}
idx.set(table[j].id,{"prev":prev,"next":undefined,"name":table[j].name.substring(0,table[j].name.length-1)});prev=table[j].id;}}}
function htLoadIndex(data,arg,page)
{if(data.content==undefined){return;}
if(page=="history"&&htHistoryIdx.has("history")==false){htFillTopIdx(htHistoryIdx,data,"history");return;}else if(page=="first_steps"&&htFirstStepsIdx.has("first_steps")==false){htFillTopIdx(htFirstStepsIdx,data,"first_steps");return;}else if(page=="literature"&&htLiteratureIdx.has("literature")==false){htFillTopIdx(htLiteratureIdx,data,"literature");return;}
