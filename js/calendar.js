...
var frMonth = [ "", "Vend�miaire", "Brumaire", "Frimaire", "Niv&ocirc;se", "Pluvi&ocirc;se", "Vent&ocirc;se", "Germinal", "Flor&eacute;al", "Prairial", "Messidor", "Thermidor", "Fructidor", "(Sans-culottides)" ];

var frDecade = [ "", "I", "II", "III" ];

var frDay = [ "", "du Primidi (1)", "du Duodi (2)", "du Tridi (3)", "du Quartidi (4)", "du Quintidi (5)", "du Sextidi (6)", "du Septidi (7)", "du Octid (8)i", "du Nonidi (9)", "du D&eacute;cadi (10)", "------------", "de la Vertu (1)", "du G&eacute;nie (2)", "du Travail (3)", "de l'Opinion (4)", "des R&eacute;compenses (5)", "de la R&eacute;volution (6)" ];
...
var indianMonths = [ "Caitra", "Vaisakha", "Jyaistha", "Asadha", "Sravana", "Bhadra", "Asvina", "Kartika", "Agrahayana", "Pausa", "Magha", "Phalguna" ];
...
//  Return the JD of the winter solstice (solar longitude 270°)
...
//  Determine whether a lunar month contains a major solar term (中气).
//  A major solar term occurs when the sun's longitude is a multiple of 30°.
...
//  Build the month map (index → {nmStart, nmEnd, hasMajor, monthNum, isLeap})
...
function jd_to_chinese(jd)
{
    ...
    // tzOffset = LOCAL - UTC in hours (UTC-3 → -3, UTC+8 → +8).
...
function calcJulian(julianDay)
{
    var j;

    j = new Number(julianDay);
    return jd_to_gregorian(j);
    /*
    var j, date, time;
...
function setJulian(julianDay)
{
    //document.julianday.day.value = new Number(j);
    return calcJulian(new Number(julianDay));
}

//  calcModifiedJulian  --  Update from Modified Julian day

function calcModifiedJulian(modifiedJulianDay)
{
    return setJulian((new Number(modifiedJulianDay)) + JMJD);
    //setJulian((new Number(document.modifiedjulianday.day.value)) + JMJD);
}

//  calcJulianCalendar  --  Update from Julian calendar

function calcJulianCalendar(julianYear, julianMonth, julianDay)
{
    return setJulian(julian_to_jd((new Number(julianYear)),
                                  julianMonth + 1,
                                  (new Number(julianDay))));
    /*
...
function calcHebrew(hebrewYear, hebrewMonth, hebrewDay)
{
    return setJulian(hebrew_to_jd((new Number(hebrewYear)),
                                  hebrewMonth + 1,
                                  (new Number(hebrewDay))));
    /*
...
function calcIslamic(islamicYear, islamicMonth, islamicDay)
{
    return setJulian(islamic_to_jd((new Number(islamicYear)),
                                   islamicMonth + 1,
                                   (new Number(islamicDay))));
    /*
...
function calcPersian(persianYear, persianMonth, persianDay)
{
    setJulian(persian_to_jd((new Number(persianYear)),
                            persianMonth + 1,
                           (new Number(persianDay))));
    /*
...
function calcPersiana(persianYear, persianMonth, persianDay)
{
    setJulian(persiana_to_jd((new Number(persianYear)),
                             persianMonth + 1,
                             (new Number(persianDay))) + 0.5);
    /*
...
function calcMayanCount(baktunValue, katunValue, tunValue, uinalValue, kinValue)
{
    return setJulian(mayan_count_to_jd((new Number(baktunValue)),
                                (new Number(katunValue)),
                                (new Number(tunValue)),
                                (new Number(uinalValue)),
                                (new Number(kin.value))));
    /*
...
function calcIndianCivilCalendar(indianYear, indianMonth, indianDay)
{
    return setJulian(indian_civil_to_jd(
                           (new Number(indianYear)),
                           indianMonth + 1,
                           (new Number(indianDay))));
    /*
...
function calcFrench(jourValue, decadeValue, moisValue, anValue)
{
    var decade, j, mois;
    ...
    */
    ...
    return setJulian(french_revolutionary_to_jd((new Number(anValue)),
                                         mois + 1,
                                         decade + 1,
                                         j + 1));
}
...
function calcExcelSerial1900(excelSerial1900)
{
    ...
    return setJulian((d - 1) + J1900);
}

//  calcExcelSerial1904  --  Perform calculation starting with an Excel 1904 serial date

function calcExcelSerial1904(excelSerial1904Day)
{
    //setJulian((new Number(document.excelserial1904.day.value)) + J1904);
    return setJulian((new Number(excelSerial1904Day)) + J1904);
}

//  calcUnixTime  --  Update from specified Unix time() value

function calcUnixTime(unixTimeSec)
{
    var t = new Number(unixTimeSec);
    return setJulian(J1970 + (t / (86400)));
    /*
