...
if(page=="tree"||page=="genealogical_map_list"||page=="indigenous_who_content"||page=="class_content"){if(reload==true&&lastTreeLoaded.arg!=null&&lastTreeLoaded.arg.length>0){arg=lastTreeLoaded.arg;}else{lastTreeLoaded.page=page;lastTreeLoaded.arg=arg;}
if(page=="tree"){$('.right-tree').css('display','block');$('.right-tree').css('visibility','visible');}}else if(page=="families"){lastTreeLoaded.page=null;lastTreeLoaded.arg=null
$("#loading").val("");$("#selector").val("");}
...
htLoadSources(data,arg,page);htFillWebPage(page,data);if(page=="main"){for(const i in data.content){htAddPaperDivs("#ht_description",data.content[i].id,data.content[i].value,"","",i);}}
return false;},});}
...
{if(data.nothing!=undefined&&data.nothing!=null&&data.nothing.length>0){$(document).prop('title',data.nothing);$("#header").html(data.nothing);return;}
...
var family_id=table.families[i].id;$("#index_list").append("<li id=\"lnk-"+family_id+"\"><a href=\"#hist-"+family_id+"\">"+keywords[8]+" : "+table.families[i].name+"</a></li>");$("#trees").append("<div id=\"hist-"+family_id+"\"></div>");var family=table.families[i];htAppendData("hist",family_id,undefined,family.name,family,page);if(family.people==undefined){continue;}
...
var destination=$("#selector").val();if(destination!=undefined&&destination!=null&&destination.length>1){var localObject=$("#name-"+destination).val();if(localObject!=undefined){$('html, body').scrollTop($("#name-"+destination).offset().top);fillTree(destination);}}
htLoadPage('tree','json','',false);$("#loading_msg").hide();}
