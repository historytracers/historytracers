var stringConstructor="HT".constructor;var vectorConstructor=[].constructor;var objectConstructor=({}).constructor;let keywords=[];let mathKeywords=[];var lastTreeLoaded={"page":null,"arg":''};var personNameMap=new Map();var familyMap=new Map();var contentMap=new Map();var primarySourceMap=new Map();var refSourceMap=new Map();var holyRefSourceMap=new Map();var smSourceMap=new Map();var genealogicalStats={};var smGame=[];var smGameTimeoutID=0;var htAtlas=[];var htHistoryIdx=new Map();var htLiteratureIdx=new Map();var htFirstStepsIdx=new Map();var htFamilyIdx=new Map();var htGameImages=["MachuPicchu.jpg","WitzXunantunich.jpg","TeotihuacanGeneral.jpg","CaralPiramideH1.jpg","PachacutiCusco.jpg","CahalPech.jpg","CaracolWitz.jpg","JoyaCeren.jpg","SanAndres.jpg","NecropoleTikal.jpg","CiudadTula.jpg","Huaca.jpg","MiPueblito.jpg","CopanAltarGenealogy0.jpg","CopanAltarGenealogy1.jpg","CopanAltarGenealogy2.jpg","CopanAltarGenealogy3.jpg","TeotihuacanMountains.jpg","CopanWholeTextStelaAltar.png","StelaACopan.jpg","Kaminaljuyu.jpg"];var htGameImagesLocation=["Machu Picchu, Perú","Xunantunich, Belieze","Teotihuacan, México","Caral, Perú","Cusco, Perú","Cahal Pech, Belieze","Caracol, Belieze","Joya de Ceren, El Salvador","San Andres, El Salvador","Tikal, Guatemala","Ciudad de Tula, México","Huaca Puclana, Perú","Mi Pueblito, Panamá","Copan, Honduras","Copan, Honduras","Copan, Honduras","Copan, Honduras","Teotihuacan, México","Copan, Honduras","Copan, Honduras","Kaminaljuyu, Guatemala"];var htSequenceGame=["CeramicaAntropologiaPeru.jpg","ChocolatPotCahalPech.jpg","EstelaAntropologiaGuatemala.jpg","Kaminaljuyu.jpg","MayaCRJade.jpg","MetateTeotihuacan.jpg","SanJoseCRAntropologia.jpg","SanSalvadorESAntropologia.jpg","StelaACopan.jpg","MusicCR.jpg"];var htSequenceGameLocation=["Lima, Peru","Cahal Pech, Belize","Ciudad de Guatemala, Guatemala","Kaminaljuyu - Ciudad de Guatemala, Guatemala","San Jose, Costa Rica","Teotihuacan, Mexico","San Jose, Costa Rica","San Salvador, El Salvador","Copan, Honduras","San Jose, Costa Rica"];var htEditable=undefined;var htEditableCheck=true;function htEnableEdition(){if(htEditableCheck==false||htEditable!=undefined){return;}
htEditableCheck=false;$("#loading_msg").hide();$("#messages").hide();var unixEpoch=Date.now();$.ajax({type:'GET',url:'edit/',contentType:'application/json; charset=utf-8',data:'nocache='+unixEpoch,async:true,dataType:'json',success:function(data){if(data.length==0){$("#loading_msg").hide();return false;}
if(data.editable!=undefined){htEditable=data.editable;htEditableCheck=true;$(".htEditor").each(function(){$(this).css('visibility','visible');});}
return false;},error:function(data){$(".htEditor").each(function(){$(this).css('visibility','hidden');});},});}
function htScroolToID(id){$('html, body').scrollTop($(id).offset().top);}
function htResetGenealogicalStats(){return{"primary_src":0,"reference_src":0,"holy_src":0,"families":0,"people":0,"marriages":0,"children":0};}
function htAddTreeReflection(id,key)
{if($(id).length>0&&keywords.length>=key){$(id).html(keywords[key]);}}
...
{var useYear="";if(year!="now"){useYear=parseInt(year);}else{var ct=new Date();useYear=ct.getFullYear();}
var dateArr=new Array(useYear,month,day);return htConvertDate(test,locale,undefined,undefined,dateArr);}
...
dateDiv+=keywords[33]+" : "+text+".";dateDiv+="</div><div id=\"paper-print\" class=\"paper-print-style\"><a href=\"#\" class=\"fa-solid fa-print\" onclick=\"htPrintContent('#header', '#page_data'); return false;\"></a></div></div><br /><i>"+keywords[24]+" "+keywords[38]+"</i></p>";$(target).append(dateDiv);}
...
if(dir.length!=0){url+=dir+"/";}
url+=page+".json";return url;}
function htOverwriteHTDateWithText(text,localDate,localLang,localCalendar){if(localDate==undefined||localDate.length==0){return text;}
var ret=text;for(const i in localDate){var changed=ret.replace("<htdate"+i+">",htMountSpecificDate(localDate[i],localLang,localCalendar));ret=changed;}
return ret;}
function htParagraphFromObject(localObj,localLang,localCalendar){var format=(localObj.format==undefined)?"html":localObj.format;var originalText="";if(localObj.fill_dates!=undefined){originalText=htOverwriteHTDateWithText(localObj.text,localObj.fill_dates,localLang,localCalendar);}else{originalText=localObj.text;}
var text=(format=="html")?"<p>":"";text+=htFormatText(originalText,format,localObj.isTable);if(localObj.source!=undefined&&localObj.source!=null){var sources=localObj.source;text+=" (";for(const i in sources){if(i!=0){text+=" ; ";}
var fcnt=htFillHistorySourcesSelectFunction(sources[i].type);var dateText=(sources[i].date!=undefined)?", "+htMountSpecificDate(sources[i].date,localLang,localCalendar):"";text+="<a href=\"#\" onclick=\"htCleanSources(); "+fcnt+"('"+sources[i].uuid+"'); return false;\"><i>"+sources[i].text+" "+dateText+"</i></a>";}
text+=")";}
text+=(localObj.PostMention!=undefined&&localObj.PostMention!=null)?localObj.PostMention:".";return text+"</p>";}
function htFillSMGameData(data){if(data==undefined||data.content.length==0){return;}
var x=document.getElementsByClassName("htSlide");if(x.length!=0){$(".htSlide").remove();}
var requestType=opt0ac0098b.requestType;var table=data.levels;if(table!=undefined&&requestType=="splash"&&$("#smGameMenu").length==0){$(opt0ac0098b.target).append("<div class=\"htSlideGameMenu htSlideGameMenuHidden\" id=\"smGameMenu\"></div> <div id=\"smGameScore\" class=\"htSlideGameScore htSlideGameMenuHidden\"><b>"+keywords[70]+"<span id=\"currentSMScore\">0</span></b></div> <div class=\"htSlideGameTextBottom smGameTextSize htSlideGameMenuHidden\" id=\"smGameDesc\"></div> <div id=\"smGamePlay\"><i class=\"fa-solid fa-play htSlidePlayGame\" onclick=\"htStartSMGame();\"></i></div>");for(const i in table){if(table[i].type==undefined){continue;}else if(table[i].type=="topLevel"){$("#smGameMenu").append("<p class=\"smGameTextSize\"><b>"+table[i].name+"</b>: "+table[i].desc+"</p><p><ul id=\""+table[i].id+"\"></ul></p>");}else if(table[i].type=="level"){if($("#"+table[i].target).length>0){$("#"+table[i].target).append("<li class=\"menu\"> <span class=\" smGameTextSize\"><a href=\"javascript:void(0);\" onclick=\"htLoadPageV1('"+table[i].loadID+"', 'json', '', 'false', 'smGame', '');\">"+table[i].name+"</a>: "+table[i].desc+"</span></li>");}}}
var imgIndex=getRandomArbitrary(0,htGameImages.length-5);$(opt0ac0098b.target).append("<p class=\"desc\"><img class=\"imgGameSizeWithOpacity\" src=\"images/"+htGameImages[imgIndex]+"\"><br />"+keywords[71]+" "+htGameImagesLocation[imgIndex]+".</p>");}
smGame=[];var table=data.content;var extClassText=(requestType=="splash")?"htSlideGameTextCenter":"htSlideGameTextTopCenter";var counter=-1;for(const i in table){var classText;if(table[i].position!=undefined){switch(table[i].position){case"center":classText="htSlideGameTextCenter";break;case"topCenter":classText="htSlideGameTextTopCenter";break;default:classText="htSlideGameTextTop";}}else{classText=extClassText;}
var useText="";var localLang=$("#site_language").val();var localCalendar=$("#site_calendar").val();if(table[i].text!=undefined){if(table[i].text.constructor===vectorConstructor){var rows=table[i].text;for(const j in rows){var commonText=(rows[j].constructor===stringConstructor)?rows[j]:htParagraphFromObject(rows[j],localLang,localCalendar);if(table[i].paragraphClass!=undefined){commonText=commonText.replace(/<p>/g,"<p class=\""+table[i].paragraphClass+"\">");}
useText+=commonText;}}else{useText=table[i].text;}}
var useDesc=null;if(table[i].desc!=undefined){if(table[i].desc.constructor===vectorConstructor){var rows=table[i].desc;useDesc="";for(const j in rows){useDesc+=rows[j];}}else{useDesc=table[i].desc;}}
smGame.push({"desc":useDesc,"prev":table[i].prev,"next":table[i].next,"jumpTo":table[i].jumpTo,"score":table[i].score,"answer":table[i].answer,"played":false});$(opt0ac0098b.target).append("<div class=\"htSlide\" id=\""+table[i].id+"\"><div class='"+classText+"'>"+useText+"</div>");counter++;}
opt0ac0098b.maxID=counter;x=document.getElementsByClassName("htSlide");if(requestType!="splash"){if($("#smGameNext").length==0){$(opt0ac0098b.target).append("<div id=\"smGameNext\"><i class=\"fa-solid fa-chevron-right htSlideNextGame\" onclick=\"htPlusSMGameDivs(1);\"></i></div> <div id=\"smGamePrev\"><i class=\"fa-solid fa-chevron-left htSlidePrevGame htSlideGameMenuHidden\" onclick=\"htPlusSMGameDivs(-1);\"></i></div> <div id=\"smGameLike\"><i class=\"fa-solid fa-thumbs-up htSlideLikeGame\" onclick=\"htSMCheckAnswer(1);\"></i></div> <div id=\"smGameUnlike\"><i class=\"fa-solid fa-thumbs-down htSlideUnlikeGame\" onclick=\"htSMCheckAnswer(0);\"></i></div>");$("#smGameLike").addClass("htSlideGameMenuHidden");$("#smGameUnlike").addClass("htSlideGameMenuHidden");$("#currentSMScore").html(totalSMScore);}else{$("#smGameNext").removeClass("htSlideGameMenuHidden");$("#smGamePrev").removeClass("htSlideGameMenuHidden");}
$("#smGameMenu").addClass("htSlideGameMenuHidden");if($("#smGameScore").hasClass("htSlideGameMenuHidden")){$("#smGameScore").removeClass("htSlideGameMenuHidden");}
htShowSlideDivs(x,0);}else{if(smGameTimeoutID!=0){clearTimeout(smGameTimeoutID);smGameTimeoutID=0;}}
$('table').each(function(){if(!$(this).hasClass('book_navigation')){$(this).addClass('three_table_bg');}});}
function htImageZoom(id,translate){var name=$("#"+id).prop("name");if(name.length==0){$("#"+id).attr("name","zoomin");$("#"+id).css("transform","scale(2.7)");$("#"+id).css("translate",translate);}else{$("#"+id).attr("name","");$("#"+id).css("transform","scale(1)");$("#"+id).css("translate","0%");}}
function htModifyAtlasIndexMap(id){var next=parseInt(id)+1;$("#atlasindex option[value="+next+"]").prop('selected',true);var myURL='index.html?page=atlas&atlas_page='+next;window.history.replaceState(null,null,myURL);$("#atlas").val(next);}
function htFormatText(text,format,table){if(format=="html"){return text;}
var converter=new showdown.Converter();if(table!=undefined&&table==true||table==1){converter.setOption('tables',true);}
var html=converter.makeHtml(text);if(html.length<4){return html;}
var htmlTest=html.substring(html.length-4);return(htmlTest=="</p>")?html.substring(0,html.length-4):html;}
function htSelectAtlasMap(id){if(htAtlas.length<id||id<0){return;}
var vector=htAtlas[id];var author="";if(vector.author!=null){if(vector.author=="HTMW"){author=keywords[82];}else{author=vector.author;}}
var formattedText=htFormatText(vector.text,vector.format,vector.isTable);if(vector.format==undefined||vector.format=="html"){formattedText="<p>"+formattedText+"</p>";}else{formattedText+="</p>";}
var text=(vector.image!=null)?"<p class=\"desc\"><img id=\"atlasimg\" src=\""+vector.image+"\" class=\"imgcenter\" onclick=\"htImageZoom('atlasimg', '-35%')\" />"+keywords[81]+" 1: "+author+".</p>"+formattedText:formattedText;var prevIdx=id-1;var nextIdx=id+1;var prevText=(prevIdx>=0)?"<a href=\"javascript:void(0);\" onclick=\"htSelectAtlasMap("+prevIdx+"); htModifyAtlasIndexMap("+prevIdx+");\">"+htAtlas[prevIdx].name+"</a>":"&nbsp;";var nextText=(nextIdx<htAtlas.length)?"<a href=\"javascript:void(0);\" onclick=\"htSelectAtlasMap("+nextIdx+"); htModifyAtlasIndexMap("+nextIdx+");\">"+htAtlas[nextIdx].name+"</a>":"&nbsp;";text+="<p><div style=\"width: 50%; float: left; font-weight: bold;\">"+keywords[56]+"<br />"+prevText+"</div><div style=\"width: 50%; float: right; font-weight: bold; text-align: right;\">"+keywords[58]+"<br />"+nextText+"</div></p><p>&nbsp;</p>";$("#rightcontent").html(text);}
function htFillAtlas(data){htAtlas=[];if(data.atlas.length==0){return;}
var localAtlas=data.atlas;var localLang=$("#site_language").val();var localCalendar=$("#site_calendar").val();for(i in localAtlas){var text="";for(const j in localAtlas[i].text){var localObj=localAtlas[i].text[j];var localtext=(localObj.text!=undefined&&localObj.source!=undefined)?htParagraphFromObject(localObj,localLang,localCalendar):localObj;text+=localtext;}
var author=(localAtlas[i].author!=undefined)?localAtlas[i].author:null;var isTable=(localAtlas[i].isTable!=undefined)?localAtlas[i].isTable:false;var format=(localAtlas[i].format!=undefined)?localAtlas[i].format:"html";htAtlas.push({"name":localAtlas[i].name,"image":localAtlas[i].image,"author":author,"text":text,"format":format,"isTable":isTable});var showIdx=parseInt(i)+1;var o=new Option(showIdx+". "+localAtlas[i].name,showIdx);$("#atlasindex").append(o);}
var idx=($("#atlas").length>0)?$("#atlas").val():0;if(isNaN(idx)||idx.length==0){htModifyAtlasIndexMap(1);idx=1;}else{idx-=1;htModifyAtlasIndexMap(idx);}
...
htLoadIndex(data,arg,page);htLoadSources(data,arg,page);htProccessData(data,optional);if(arg!=source){htEnableEdition();}
return false;},});}
...
switch(page){case"tree":case"genealogical_map_list":case"class_content":if(reload==true&&lastTreeLoaded.arg!=null&&lastTreeLoaded.arg.length>0){arg=lastTreeLoaded.arg;htHistoryIdx.clear();htLiteratureIdx.clear();htFirstStepsIdx.clear();htFamilyIdx.clear();}else{lastTreeLoaded.page=page;lastTreeLoaded.arg=arg;}
if(page=="tree"){$('.right-tree').css('display','block');$('.right-tree').css('visibility','visible');}
var myURL=(arg!=undefined&&arg!=null)?'index.html?page='+page+'&arg='+arg:'index.html?page='+page;genealogicalStats=htResetGenealogicalStats();window.history.replaceState(null,null,myURL);break;case"families":lastTreeLoaded.page=null;lastTreeLoaded.arg=null
...
htLoadIndex(data,arg,page);htLoadSources(data,arg,page);htFillWebPage(page,data);if(arg!="source"){htEnableEdition();}
...
{switch(dateObj.type){case"gregory":if(dateObj.day>0){updateText=htConvertGregorianDate(localCalendar,localLang,dateObj.year,dateObj.month,dateObj.day);}else{updateText=htConvertGregorianYear(localCalendar,dateObj.year);}
break;case"unix":updateText=htConvertUnixDate(localCalendar,localLang,dateObj.epoch);break;case"julian":updateText=htConvertJulianDate(localCalendar,localLang,dateObj.day);break;}
return updateText;}
...
var w=vector[j++];var updateText=htMountSpecificDate(w,localLang,localCalendar);$(this).html(updateText);});}
...
if(data.common!=undefined&&data.common!=null&&data.common.length>0){var localLang=$("#site_language").val();var localCalendar=$("#site_calendar").val();for(const i in data.common){var commonObj=data.common[i];var commonText=(commonObj.constructor===stringConstructor)?commonObj:htParagraphFromObject(commonObj,localLang,localCalendar);$("#common").append(commonText);}}
...
continue;}
if(data.content[i].value.constructor===stringConstructor){if($("#"+data.content[i].id).length>0){$("#"+data.content[i].id).html(data.content[i].value);}else if((page=="families"||page=="history"||page=="literature"||page=="first_steps")&&(data.content[i].target!=undefined)){$("#group-map").append("<ul><b><span id=\""+data.content[i].id+"\">"+data.content[i].value+"</span></b><ol id=\""+data.content[i].target+"\"></ol></ul><br />");}}else if(data.content[i].value.constructor===vectorConstructor&&data.content[i].target!=undefined){if(data.content[i].value_type==undefined){continue;}else if(data.content[i].value_type=="family-list"){var table=data.content[i].value;htFillFamilyList(table,data.content[i].target);}else if(data.content[i].value_type=="group-list"){if(data.content[i].id!=undefined&&data.content[i].id!=null&&data.content[i].id.length>0&&data.content[i].desc!=undefined&&data.content[i].desc.length>0){$("#"+data.content[i].id).html(data.content[i].desc);}
htFillMapList(data.content[i].value,data.content[i].target,data.content[i].page);}else if(data.content[i].value_type=="subgroup"){htFillSubMapList(data.content[i].value,data.content[i].target);}else if(data.content[i].value_type=="paper"){htFillPaperContent(data.content[i].value,last_update,page_authors,page_reviewers,data.index);}}else if(data.content[i].value.constructor===vectorConstructor&&data.content[i].id!=undefined){if(data.content[i].id!="fill_dates"){for(const j in data.content[i].value){$("#"+data.content[i].id).append(data.content[i].value[j]);}}else{if(data.content[i].value.constructor===vectorConstructor){htFillHTDate(data.content[i].value);}}}}
if(data.atlas!=undefined){htFillAtlas(data);}
...
if($("#tree-description").length>0){$("#tree-description").html(" <p>"+keywords[52]+"</p>");}}
...
if($("#htZoomImageMsg").length>0){$("#htZoomImageMsg").html(keywords[84]);}
if($("#htAmericaAbyaYalaMsg").length>0){$("#htAmericaAbyaYalaMsg").html(keywords[85]);}
...
{if(index=="history"){return htHistoryIdx;}else if(index=="literature"){return htLiteratureIdx;}else if(index=="first_steps"){return htFirstStepsIdx;}else if(index=="families"){return htFamilyIdx;}
return undefined;}
function htBuildNavigation(index)
{var urlParams=new URLSearchParams(window.location.search);if(!urlParams.has('arg')){return null;}
var arg=urlParams.get('arg');var idx=htSelectIndexMap(index);var ptr=idx.get(arg);if(ptr==undefined){return null;}
var pageName=(index=="families")?"tree":"class_content";var prev="";if(ptr.prev==index){prev="<a href=\"index.html?page="+index+"\" onclick=\"htLoadPage('"+index+"','html', '', false); return false;\"><span>"+keywords[60]+"</span></a>";}else{var prevPtr=idx.get(ptr.prev);prev="<a href=\"index.html?page="+pageName+"&arg="+ptr.prev+"\" onclick=\"htLoadPage('"+pageName+"', 'html', '"+ptr.prev+"', false); return false;\">"+prevPtr.name+"</a>";}
var next="";if(ptr.next==undefined){next="&nbsp;";}else{var nextPtr=idx.get(ptr.next);next="<a href=\"index.html?page="+pageName+"&arg="+ptr.next+"\" onclick=\"htLoadPage('"+pageName+"', 'html', '"+ptr.next+"', false); return false;\">"+nextPtr.name+"</a>";}
var navigation="<p><table class=\"book_navigation\"><tr><td><span>"+keywords[56]+"</span></td> <td> <span>"+keywords[57]+"</span> </td> <td><span>"+keywords[58]+"</span></td></tr><tr><td>"+prev+"</td> <td><a href=\"index.html?page="+index+"\" onclick=\"htLoadPage('"+index+"','html', '', false); return false;\"><span>"+keywords[60]+"</span></td><td>"+next+"</td></tr></table></p>";return navigation;}
function htWriteNavigation(index)
{var navigation=htBuildNavigation(index);$(".dynamicNavigation").each(function(){$(this).html(navigation);});}
function htFillTopIdx(idx,data,first)
{var prev=first;idx.set(first,{"prev":first,"next":undefined,"name":keywords[57]});for(const i in data.content){if(data.content[i].value.constructor!==vectorConstructor||data.content[i].page==undefined){continue;}
var table=data.content[i].value;var name="";for(const j in table){var fillNext=idx.get(prev);if(fillNext!=undefined){fillNext.next=table[j].id;}
var pos=table[j].name.length-1;if(first=="families"){var testing=table[j].name.search("\\(<span");if(testing>0){pos=testing;}}
idx.set(table[j].id,{"prev":prev,"next":undefined,"name":table[j].name.substring(0,pos-1)});prev=table[j].id;}}}
function htLoadIndex(data,arg,page)
{if(data.content==undefined&&data.families==undefined){return;}
if(page=="history"&&htHistoryIdx.has("history")==false){htFillTopIdx(htHistoryIdx,data,"history");return;}else if(page=="first_steps"&&htFirstStepsIdx.has("first_steps")==false){htFillTopIdx(htFirstStepsIdx,data,"first_steps");return;}else if(page=="literature"&&htLiteratureIdx.has("literature")==false){htFillTopIdx(htLiteratureIdx,data,"literature");return;}else if(page=="families"&&htFamilyIdx.has("families")==false){htFillTopIdx(htFamilyIdx,data,"families");return;}
if(data.index==undefined){return;}
var URL=htLoadPageMountURL(data.index,data.index,"");var unixEpoch=Date.now();$.ajax({type:'GET',url:URL,contentType:'application/json; charset=utf-8',data:'nocache='+unixEpoch,async:true,dataType:'json',success:function(d){if(d.length==0){return false;}
htLoadIndex(d,arg,data.index);return false;},});}
...
$("#htAnswersToBeUsed").html("");return ret;}
function htFillPaperContent(table,last_update,page_authors,page_reviewers,index){var localLang=$("#site_language").val();var localCalendar=$("#site_calendar").val();$("#paper").html("<i>"+keywords[87]+"</i>");var navigationPage=table[0].text;var idx=0;var later="";for(const i in table){if(i==1){htFillDivAuthorsContent("#paper",last_update,page_authors,page_reviewers);}
if(index&&i==0){navigationPage="<p class=\"dynamicNavigation\"></p>";htAddPaperDivs("#paper","indexTop",navigationPage,"","<hr class=\"limit\" />",idx);idx++;later="";}else{if(table[i].id=="navigation"){navigationPage=table[0].text;}
later=(i==0&&last_update>0&&table[i].id=="navigation")?"<hr class=\"limit\" />":"";}
if(table[i].text.constructor===stringConstructor){htAddPaperDivs("#paper",table[i].id,table[i].text,"",later,idx);}else if(table[i].text.constructor===vectorConstructor){if(table[i].id=="exercise_v2"){htWriteQuestions(table[i].text,later,idx);}else if(table[i].id=="game_v1"){htWriteGame(table[i].text,later,idx);}else if(table[i].id!="fill_dates"){for(const j in table[i].text){var localObj=table[i].text[j];var text=(localObj.text!=undefined)?htParagraphFromObject(localObj,localLang,localCalendar):localObj;htAddPaperDivs("#paper",table[i].id+"_"+j,text,"",later,idx);}}else{htFillHTDate(table[i].text);}}
idx++;}
if(navigationPage.length>0){htAddPaperDivs("#paper","repeat-index",navigationPage,"<hr class=\"limit\" />","",100000);}}
...
var people=family.people;totalPeople+=people.length;for(const j in people){if(people[j].id==undefined||people[j].name==undefined){continue;}
...
genealogicalStats.people=totalPeople;var destination=$("#selector").val();if(destination!=undefined&&destination!=null&&destination.length>1){var localObject=$("#name-"+destination).val();if(localObject!=undefined){htScroolToID("#name-"+destination);htFillTree(destination);}}
...
function htAppendData(prefix,id,familyID,name,table,page){var history=table.history;var parents=table.parents;var marriages=table.marriages;if(history!=undefined){var title;var localHeader;var goToTop;if((parents==undefined||marriages==undefined)&&(prefix!="tree")){title=keywords[8];localHeader="3";goToTop="<a href=\"javascript:void(0);\" onclick=\"htScroolToID('#index_list');\">"+keywords[78]+"</a>";}else{title=keywords[9];localHeader="4";goToTop="";}
...
var primary_source=table.primary_source;var references=table.references;var holy_references=table.holy_references;htFillHistorySources(id,"#"+prefix+"-"+id,history,"tree-default-align",id);var global_father=null;if(parents!=undefined){for(const i in parents){var couple=parents[i];var parents_id=prefix+"-parents-"+id;var father=(couple.father_id)?couple.father_id:couple.father;var mother=(couple.mother_id)?couple.mother_id:couple.mother;if(father==null&&mother==null){$("#"+prefix+"-"+id).append("<div id=\""+parents_id+"\" class=\"tree-real-family-text\"><p><b>"+keywords[0]+"</b>: "+keywords[10]+"</p></div>");familyMap.set(id,"null&null&t");}else{var parentsLink="";var name="";if(father!=undefined&&father!=null){global_father=father;parents_id+=father+"-";name=personNameMap.get(father);if(name!=undefined){parentsLink+="<a href=\"javascript:void(0);\" onclick=\"htScroolTree('#name-"+father+"'); htFillTree('"+father+"'); htSetCurrentLinkBasis('"+page+"', '"+father+"',"+undefined+");\">"+name+"</a> ";}else if(couple.father_name!=undefined&&couple.father_family!=undefined&&couple.father_family!=familyID){parentsLink+="<a href=\"index.html?page=tree&arg="+couple.father_family+"&person_id="+father+"&lang="+$('#site_language').val()+"&cal="+$('#site_calendar').val()+"\" onclick=\"htLoadPage('tree', 'html', '"+couple.father_family+"&person_id="+father+"', false); return false;\">"+couple.father_name+"</a> & ";}}
parents_id+="-";if(mother!=null&&mother!=undefined){parents_id+=mother+"-";name=personNameMap.get(mother);if(name!=undefined){if(couple.mother_family!=undefined){if(couple.mother_family==familyID||(couple.mother_external_family_file!=undefined&&couple.mother_external_family_file==false)){parentsLink+=" & <a href=\"javascript:void(0);\" onclick=\"htScroolTree('#name-"+mother+"'); htFillTree('"+mother+"'); htSetCurrentLinkBasis('"+page+"', '"+mother+"',"+undefined+");\">"+name+"</a>";}else{parentsLink+="<a href=\"index.html?page=tree&arg="+couple.mother_family+"&person_id="+mother+"&lang="+$('#site_language').val()+"&cal="+$('#site_calendar').val()+"\" onclick=\"htLoadPage('tree', 'html', '"+couple.mother_family+"&person_id="+mother+"', false); return false;\">"+name+"</a>";}}else{parentsLink+=" & "+name;}}else if(couple.mother_name!=undefined&&couple.mother_family!=undefined&&couple.mother_family!=familyID){parentsLink+="<a href=\"index.html?page=tree&arg="+couple.mother_family+"&person_id="+mother+"&lang="+$('#site_language').val()+"&cal="+$('#site_calendar').val()+"\" onclick=\"htLoadPage('tree', 'html', '"+couple.mother_family+"&person_id="+mother+"', false); return false;\">"+couple.mother_name+"</a>";}}
...
if(marriages!=undefined){genealogicalStats.marriages+=marriages.length;for(const i in marriages){var marriage=marriages[i];var rel_id=prefix+"-relationship-"+marriage.id;var marriage_class;var type=marriage.type;var official=marriage.official;var marriage_keyword;if(marriage.id==undefined){$("#"+prefix+"-"+id).append("<div id=\""+rel_id+"\" class=\"tree-real-family-text\"><p><b>"+keywords[17]+"</b>: "+keywords[19]+"</p></div>");}else{if(type=="theory"){marriage_class="tree-real-family-text";marriage_keyword=keywords[17];}else{marriage_class="tree-hipothetical-family-text";marriage_keyword=keywords[18];}
if(official!=undefined&&official==false){marriage_keyword=keywords[86];}
var marriageLink="";if(marriage.family_id==undefined){marriageLink=marriage.name;}else if(familyID==marriage.family_id||(marriage.external_family_file!=undefined&&marriage.external_family_file==false)){marriageLink="<a href=\"javascript:void(0);\" onclick=\"htScroolTree('#name-"+marriage.id+"'); htFillTree('"+marriage.id+"'); htSetCurrentLinkBasis('"+page+"', '"+marriage.id+"',"+undefined+");\">"+marriage.name+"</a>";}else{marriageLink="<a href=\"index.html?page=tree&arg="+marriage.family_id+"&person_id="+marriage.id+"&lang="+$('#site_language').val()+"&cal="+$('#site_calendar').val()+"\" onclick=\"htLoadPage('tree', 'html', '"+marriage.family_id+"&person_id="+marriage.id+"', false); return false;\">"+marriage.name+"</a>";}
$("#"+prefix+"-"+id).append("<div id=\""+rel_id+"\" class=\""+marriage_class+"\"><p><b>"+marriage_keyword+"</b>: "+marriageLink+".</p></div>");htFillHistorySources(marriage.id,"#"+rel_id,marriage.history,"tree-default-align",marriage.id);var showTree=personNameMap.has(marriage.id);if(showTree==false){personNameMap.set(marriage.id,marriage.name);}}}}
var children=table.children;if(children!=undefined){genealogicalStats.children+=children.length;for(const i in children){var child=children[i];var child_id=prefix+"-children-"+child.id;var relationship_id=prefix+"-relationship-";if(child.marriage_id!=undefined){relationship_id+=child.marriage_id;}
...
{var localLang=$("#site_language").val();var localCalendar=$("#site_calendar").val();if(history!=undefined){for(const i in history){var localObj=history[i];var text=(localObj.text!=undefined&&localObj.source!=undefined)?htParagraphFromObject(localObj,localLang,localCalendar):"<p>"+localObj+"</p>";$(histID).append("<p class=\""+useClass+"\" onclick=\"htFillTree('"+personID+"'); \">"+text+"</p>");}}}
...
{$("#tree-source").html("");$("#tree-ref").html("");$("#tree-holy-ref").html("");$("#tree-sm-ref").html("");}
...
function htShowSlideDivs(x,index){if(x==undefined){return;}
for(let i=0;i<x.length;i++){x[i].style.display="none";}
x[index].style.display="block";}
function htShowSlideDivsAuto(x,index,stopMax){if(x==undefined){return;}
for(let i=0;i<x.length;i++){x[i].style.display="none";}
x[index].style.display="block";++index;if(stopMax==true&&index==x.length){return 0;}
if(index==x.length){index=0;}else if(index<0){index=x.length-1;}
