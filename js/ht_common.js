...
function htLoadPixQRCode(id, perc) {
    var htmlLength = $("#"+id).html();
    if (htmlLength.length == 0) {
        $("#"+id).html("<img src=\"images/HistoryTracers/qrcodePix.png\" id=\"myQrCode\" width=\""+perc+"\" />");
    }
}
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if ($("#htPixQRCode").length > 0 ) {
                htLoadPixQRCode("htPixQRCode", "10%");
            }

            if ($("#htPixSideQRCode").length > 0 ) {
                htLoadPixQRCode("htPixSideQRCode", "25%");
            }
            return false;
        },
    });
