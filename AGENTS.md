...
12. **`<htdateN>` sequential vector** (`767d85cd-bd1c-43af-9d86-9f5f14bf7de1` Copan – 13 slides, previously limited to 2 values):
    ...
    - Algorithm expects sequential `0..N-1` with `N>2` (one entry per `<htdateN>` distinct). Fix: strip ` (<htdateN> - <htdateM>)` from `Related content` `li` (e.g. `Indigenous (Copan) (<htdate0> - <htdate1>) - …` → `Indigenous (Copan) - …`, only captions keep `<htdate>`), assign each `htSlideCaption` a distinct `<htdate{i}>` (`0..12` for 13 slides), and expand `date_time` to same length (`13× {"type":"gregory","year":"2015","month":"04","day":"26"}`). Validate `python3 -c "import re,json; txt=json.load(open('lang/en-US/<uuid>.json'))['content'][1]['text'][1]['text']; dts=json.load(open('lang/en-US/<uuid>.json'))['content'][1]['text'][1]['date_time']; cites=set(re.findall(r'<htdate\\d+>',txt)); print(len(dts), len(cites), sorted(cites), dts==len(cites) and cites==set(f'<htdate{i}>' for i in range(len(dts))))"` → `13 13 True`, and `wc -l` identity across `en-US/es-ES/pt-BR` (now `941` lines).
