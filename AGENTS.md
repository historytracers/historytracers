...
13. **`SECTION_prerequisites` minimal content** (e.g. `3ff47bc4-4eac-4667-8a79-380ea1687bd0` Guatemala Antropologia):
   - Gallery `content[0]` (`SECTION_prerequisites`) must be exactly:
     ...
     ```
     with `source:null` and `date_time:null` (single entry, no citations). This is the correct state for all galleries **unless** the gallery text explicitly mentions all three regions — China (`htChinaZhongguo`), Japan (`htJapanNipponNihonKoku`) and Abya Yala (`htAmericaAbyaYalaMsg`/`htAgeMsg`) — in that case the full block with those spans and its citation (e.g. British Museum `6487ffc3` retains `<p><span id=\"htZoomImageMsg\"></span></p><p><span id=\"htAgeMsg\"></span></p><p><span id=\"htAmericaAbyaYalaMsg\"></span> (<htcite0>).</p><p style=\"font-style: italic;\" id=\"htChinaZhongguo\"></p><p style=\"font-style: italic;\" id=\"htJapanNipponNihonKoku\"></p>`) must be kept. Full block is required only when all three are mentioned, not when only one or two appear. For `3ff47bc4` the cleanup to the minimal block is correct and must not be restored.
